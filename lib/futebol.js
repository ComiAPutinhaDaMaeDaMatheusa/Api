  ...
  try {
    ...
    $('.feed-post').each((i, element) => {
      try {
        ...
        const manchete = $(element).find('.feed-post-body-resumo p[elementtiming="text-ssr"]').text();
        const postagem = $(element).find(seletorHorario).text();
        const linkimg = $(element).find(seletorImg).attr('src');
        const noticiaUrl = $(element).find('a.feed-post-link').attr('href');
        noticias.push({ titulo, manchete, postagem, linkimg, noticiaUrl });
      } catch (error) {
